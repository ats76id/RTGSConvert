...
uses
  ...
  StdCtrls, ADODB, AdvPanel;

type
  TfmMainUMC = class(TForm)
    AdvPanel1: TAdvPanel;
    Splitter1: TSplitter;
    Label1: TLabel;
...
procedure TfmMainUMC.acExitExecute(Sender: TObject);
begin
  Close;
end;

procedure TfmMainUMC.FormCreate(Sender: TObject);
begin
  Application.OnShowHint := ShowHint;
  Application.OnHint := Hint;
end;

procedure TfmMainUMC.FormShow(Sender: TObject);
begin
  FbtLastLevelIndex := 3;
  if ruleUserAccessList(FUserName, 6, FMyRights)=VR_FAILED then
  begin
    //SetOfRights := [];
  ...
  tvMain.Items[0].Text := rsTxtAPOKUserGroup;
...
procedure TfmMainUMC.Hint(Sender: TObject);
begin
  sbMain.Panels[0].Text := Application.Hint;
end;

procedure TfmMainUMC.ShowHint(var HintStr: string; var CanShow: Boolean;
  var HintInfo: THintInfo);
begin
...
procedure TfmMainUMC.acOptionsExecute(Sender: TObject);
begin
  fmOptions.ShowModal;
end;

procedure TfmMainUMC.InitAllFormsFirst;
begin
...
procedure TfmMainUMC.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TfmMainUMC.acNewGroupExecute(Sender: TObject);
begin
...
procedure TfmMainUMC.acRefreshExecute(Sender: TObject);
begin
  tvMainChange(Self, tvMain.Selected);
end;

procedure TfmMainUMC.tvMainChange(Sender: TObject; Node: TTreeNode);
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        tempColumn.Caption := rsTxtAPOKUserGroup;
...
procedure TfmMainUMC.tvMainExpanded(Sender: TObject; Node: TTreeNode);
begin
...
procedure TfmMainUMC.acBackExecute(Sender: TObject);
begin
  if tvMain.TopItem.Expanded then
  begin
    tvMain.FullCollapse;
...
procedure TfmMainUMC.pmTreePopup(Sender: TObject);
var
  tempMenu:TMenuItem;
begin
  pmTree.Items.Clear;
...
procedure TfmMainUMC.lvMainChange(Sender: TObject; Item: TListItem;
  Change: TItemChange);
begin
...
procedure TfmMainUMC.lvMainDblClick(Sender: TObject);
begin
...
procedure TfmMainUMC.FillAccessRight;
const
...
procedure TfmMainUMC.SetRights(Level, Index: Integer);
begin
...
procedure TfmMainUMC.lvMainKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmMainUMC.lvMainKeyPress(Sender: TObject; var Key: Char);
begin
  if Key=#13 then
    lvMainDblClick(Sender);
end;

procedure TfmMainUMC.lvMainSelectItem(Sender: TObject; Item: TListItem;
  Selected: Boolean);
begin
  if Selected then
  begin
...
procedure TfmMainUMC.acRemoveUserExecute(Sender: TObject);
var
  intI: Integer;
...
procedure TfmMainUMC.acRemoveGroupExecute(Sender: TObject);
var
  intI: Integer;
...
procedure TfmMainUMC.pmListPopup(Sender: TObject);
var
  tempMenu: TMenuItem;
  Node: TTreeNode;
begin
...
procedure TfmMainUMC.acManageUserExecute(Sender: TObject);
begin
  with fmManageUser do
  begin
...
procedure TfmMainUMC.acNewUserExecute(Sender: TObject);
begin
  if fmNewUser.ShowModal=mrOK then
    acRefresh.Execute;
end;

procedure TfmMainUMC.acManageGroupExecute(Sender: TObject);
begin
...
function TfmMainUMC.CekMyRights(intValue: Integer): Boolean;
var
  intI: Integer;
begin
  Result := False;
...
procedure TfmMainUMC.FormResize(Sender: TObject);
begin
  sbMain.Panels[0].Width := sbMain.Width - sbMain.Panels[1].Width-sbMain.Panels[2].Width;
end;

procedure TfmMainUMC.sbMainDrawPanel(StatusBar: TStatusBar;
  Panel: TStatusPanel; const Rect: TRect);
begin
  with sbMain.Canvas do
  begin
    imageList1.Draw(sbMain.Canvas, Rect.Left+2, Rect.Top+1, 1);
    TextOut(Rect.Left + 20, Rect.Top+3, FUserName);
  end;
end;

procedure TfmMainUMC.acUserListExecute(Sender: TObject);
begin
...
procedure TfmMainUMC.acGroupListExecute(Sender: TObject);
begin
...
procedure TfmMainUMC.acUserRightsExecute(Sender: TObject);
begin
//
end;

procedure TfmMainUMC.acGroupRightsExecute(Sender: TObject);
begin
...
procedure TfmMainUMC.acUserAuditExecute(Sender: TObject);
begin
